...
function createServer(protocol, defaultPort, opts) {
  _[protocol].createServer(opts, function(sreq, sres) {
    const [ hostname, port ] = `${sreq.headers.host}:${defaultPort}`.split(':', 2);
    ...
    sreq.pipe(creq);
  }).listen(defaultPort, '0.0.0.0', () => {
    console.log(`${protocol} server is listening on port ${defaultPort}.`);
  });
}
